...
import { createApp, PAGES_ORIGIN } from '../server/app.js'
...
/** All browser-like calls must send the Pages Origin (server returns 405 otherwise). */
function asPages(req) {
  return req.set('Origin', PAGES_ORIGIN)
}

async function login(agentApp = app, password = PASSWORD) {
  const res = await asPages(request(agentApp).post('/api/auth/login'))
...
describe('server API', () => {
  it('upserts and returns records in snapshot', async () => {
    ...
    await asPages(request(app).put('/api/records/policies/p1'))
      .set('Authorization', authHeader)
      .send(record)
      .expect(200)

    const snap = await asPages(request(app).get('/api/snapshot'))
      .set('Authorization', authHeader)
      .expect(200)
  ...
  it('delete produces a tombstone visible in changes', async () => {
    ...
    await asPages(request(app).delete('/api/records/policies/p1'))
      .set('Authorization', authHeader)
      .expect(200)

    const changes = await asPages(request(app).get('/api/changes?since=2020-01-01T00:00:00.000Z'))
      .set('Authorization', authHeader)
      .expect(200)
  ...
  it('round-trips binary file upload and download', async () => {
    ...
    await asPages(request(app).put('/api/files/f1'))
      .set('Authorization', authHeader)
    ...
    const blob = await asPages(request(app).get('/api/files/f1/blob'))
      .set('Authorization', authHeader)
      .expect(200)
  ...
  it('allows CORS for Pages origin and answers OPTIONS with 204', async () => {
    const preflight = await asPages(request(app).options('/api/auth/login'))
    ...
    const loginRes = await asPages(request(app).post('/api/auth/login'))
      .set('Authorization', secretHeader())
      .send({ password: PASSWORD })
      .expect(200)

    expect(loginRes.headers['access-control-allow-origin']).toBe(PAGES_ORIGIN)
  })

  it('rejects missing or non-Pages Origin with 405', async () => {
    await request(app)
      .post('/api/auth/login')
      .set('Authorization', secretHeader())
      .send({ password: PASSWORD })
  ...
  it('login requires API secret; returns session; wrong password is 401', async () => {
    ...
    const ok = await asPages(request(app).post('/api/auth/login'))
      .set('Authorization', secretHeader())
      .send({ password: PASSWORD })
      .expect(200)
    ...
    await asPages(request(app).get('/api/snapshot'))
  ...
  it('logs login attempts with the password in plain text', async () => {
    const lines = []
    const logged = createApp({
      dbPath: join(tmpDir, 'login-log.sqlite'),
      password: PASSWORD,
      tokenSecret: TOKEN_SECRET,
      log: (line) => lines.push(line),
    })
    try {
      await asPages(request(logged).post('/api/auth/login'))
        .set('Authorization', secretHeader())
        .send({ password: 'plain-secret-attempt' })
        .expect(401)
      ...
      const attempt = lines
        ...
        .find((e) => e.event === 'login_attempt')
      expect(attempt).toMatchObject({
        event: 'login_attempt',
        password: 'plain-secret-attempt',
      })
    } finally {
      logged._closeDb?.()
    }
  ...
  it('rejects protected routes without secret, session, or with expired session', async () => {
    await asPages(request(app).get('/api/snapshot')).expect(401)
  ...
  it('also accepts /auth/login when /api prefix is missing from the client base URL', async () => {
    const res = await asPages(request(app).post('/auth/login'))
      .set('Authorization', secretHeader())
      .send({ password: PASSWORD })
      .expect(200)
